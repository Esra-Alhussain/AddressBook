...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private String mType = Contacts.ContactEntry.TYPEOFCONTACT_PERSONAL;
    ImageView mPhoto;
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    if (selection.equals(getString(R.string.homephone))) {
                        mType = Contacts.ContactEntry.TYPEOFCONTACT_HOME;
                    } else if (selection.equals(getString(R.string.workphone))) {
                        mType = Contacts.ContactEntry.TYPEOFCONTACT_WORK;

                    } else {
                        mType = Contacts.ContactEntry.TYPEOFCONTACT_PERSONAL;
                    }
                }
            }
    ...
    private boolean saveContact() {
        ...
        //save it from crashing when fields are empty
        if (mCurrentContactUri == null && TextUtils.isEmpty(name)
                && TextUtils.isEmpty(email) && TextUtils.isEmpty(phone) && mType == Contacts.ContactEntry.TYPEOFCONTACT_PERSONAL && mPhotoUri == null) {
        ...
        if (TextUtils.isEmpty(name)) {
            ...
            values.put(Contacts.ContactEntry.COLUMN_NAME, name);
        }
        ...
        if (TextUtils.isEmpty(email)) {
            ...
            values.put(Contacts.ContactEntry.COLUMN_EMAIL, email);
        }
        ...
        if (TextUtils.isEmpty(phone)) {
            ...
            values.put(Contacts.ContactEntry.COLUMN_PHONENUMBER, phone);
        }


        values.put(Contacts.ContactEntry.COLUMN_TYPEOFCONTACT, mType);
        values.put(Contacts.ContactEntry.COLUMN_PICTURE, mPhotoUri.toString());

        if (mCurrentContactUri == null) {

            Uri newUri = getContentResolver().insert(Contacts.ContactEntry.CONTENT_URI, values);
            if (newUri == null) {
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        String[] projection = {Contacts.ContactEntry._ID,
                Contacts.ContactEntry.COLUMN_NAME,
                Contacts.ContactEntry.COLUMN_EMAIL,
                Contacts.ContactEntry.COLUMN_PICTURE,
                Contacts.ContactEntry.COLUMN_PHONENUMBER,
                Contacts.ContactEntry.COLUMN_TYPEOFCONTACT
        };
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            int name = cursor.getColumnIndex(Contacts.ContactEntry.COLUMN_NAME);
            int email = cursor.getColumnIndex(Contacts.ContactEntry.COLUMN_EMAIL);
            int type = cursor.getColumnIndex(Contacts.ContactEntry.COLUMN_TYPEOFCONTACT);
            int number = cursor.getColumnIndex(Contacts.ContactEntry.COLUMN_PHONENUMBER);
            int picture = cursor.getColumnIndex(Contacts.ContactEntry.COLUMN_PICTURE);
            ...
            switch (typeof) {
                ...
                case Contacts.ContactEntry.TYPEOFCONTACT_HOME:
                    mSpinner.setSelection(1);
                    break;

                case Contacts.ContactEntry.TYPEOFCONTACT_WORK:
